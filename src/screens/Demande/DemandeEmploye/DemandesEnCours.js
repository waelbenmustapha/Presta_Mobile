...
import { FlatList,Image, Picker,Modal, View, StyleSheet, Text, SafeAreaView, ScrollView, TouchableOpacity, RefreshControl } from 'react-native';
...
export default function AcceptationDemande({navigation}) {
  ...
  const [demandetype,setdemandetype]=useState('Demandes Terminees');
...
await axios.get("http://192.168.43.100:8090/demandeur/consulterDemandeur/"+tokeen).then(response => {
  ...
  async function demandesenattente(id) {
    ...
    axios.get("http://192.168.43.100:8090/demande/demandeEmploye/"+id+"/2", {}, {
      auth: {
        username: "user",
...
<Picker.Item label={"Demandes annulées"} value={"DemandesAnnulees"} />
            ...
            <FlatList
              ...
              <View
                  ...
                  <TouchableOpacity style={{ flex: 1 }} onPress={() => { funcsaccepterourefuser(item.id, 3);getconnecteduser(); sendPushNotification(item.demandeur.notification_token, item.nom, "L'exécution de la demande de " + item.nom + " est terminée"); }}>
                      <View style={styles.start} >

                        <Text style={styles.textSign} >Terminer</Text>
                      </View>
                    </TouchableOpacity>
                  <TouchableOpacity style={{ flex: 1 }} onPress={() => { funcsaccepterourefuser(item.id, -1);getconnecteduser(); sendPushNotification(item.demandeur.notification_token, item.nom, "L'exécution de la demande de " + item.nom + " est annulée"); }}>
                      <View style={styles.start} >
