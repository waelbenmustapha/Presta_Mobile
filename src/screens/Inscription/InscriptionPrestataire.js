...
import {ActivityIndicator, Modal,View, Text, StyleSheet, Platform, TextInput, ScrollView, Alert, TouchableOpacity, Picker } from 'react-native';
import {FontAwesome, Feather, MaterialCommunityIcons, MaterialIcons,FontAwesome5, AntDesign} from '@expo/vector-icons';
...
const [errorMsg, setErrorMsg] = useState(null);
    ...
    const [picturetwo, setPicturetwo] = React.useState(null)
    ...
    const handleUploadOne = (image) => {
        ...
        axios.post
            ("https://api.cloudinary.com/v1_1/presta/image/upload", data)
            .then((response) => {
    ...
    const handleUploadTwo = (image) => {
        ...
        axios.post
            ...
            .then((response) => {
                console.log(response.data);
                setPicturetwo(response.data.url);
            }
            )
            .catch(err => {
                console.log(err)
    ...
    let openImagePickerAsyncTwo = async () => {
        const { granted } = await Permissions.askAsync(Permissions.CAMERA_ROLL)
        if (granted) {
            let data = await ImagePicker.launchImageLibraryAsync({
                mediaTypes: ImagePicker.MediaTypeOptions.Images,
    ...
    const axiosApiCallEmail = () => {
        ...
        axios.post("http://192.168.43.100:8090/demandeur/processRegister", email, config)
            .then((response) => {
                navigation.navigate('InscriptionPrestataireEmail',
                    {
                        password: password, nom: nom, phone: phone, email: email,
                        service: selectedservice,
                        code: response.data,
                        image1: pictureone , image2: picturetwo ,
                        marker:marker
                    });
            })
            .catch((error) => {
    ...
    const axiosApiCall = () => {
        ...
        axios.post("http://192.168.43.100:8090/mobilenumbers/otp", phone, config)
            .then((response) => {
                ...
                navigation.navigate('InscriptionPrestataireTelephone',
                    {
                        password: password, nom: nom, phone: phone, email: email, 
                        service: selectedservice,
                        image1: pictureone , image2: picturetwo ,marker:marker
                    });
            })
            .catch((error) => {
                console.log(error)
    ...
    const verifyExistanceEmail = () => {
        ...
        axios.post("http://192.168.43.100:8090/prestataire/prestataireExistant", email, config)
            .then((response) => {
    ...
    const onPressContinuePhone = () => {
        ...
        if (check_emailInputChange && check_textInputChange && check_passwordInputChange && check_phoneInputChange
            && (pictureone != null && picturetwo != null&& marker!=null))
        {
            console.log(pictureone,picturetwo)
            verifyExistance();
        }
      ...
      onLongPress={(e) => setMarker(e.nativeEvent.coordinate)}>
{
     ...
     <View style={{position:'absolute',bottom:10,right:0,left:0}} ><TouchableOpacity style={{borderWidth:1,borderColor:'white',backgroundColor:'#45d3f4',borderRadius:15,justifyContent:'center',alignSelf:'center',height:50,width:160}} onPress={()=>{setModalVisible(false)}}><Text style={{textAlign:'center',color:'white',fontWeight:'bold'}}>sélectionner position</Text></TouchableOpacity></View>
            ...
            <View style={styles.footer}>
                <ScrollView>
                    <Text style={styles.titrePick}>Choisissez la catégorie et le service que vous offrez </Text>
                    <View style={styles.action}>
                        <View style={{ borderWidth: 1, borderColor: '#45d3f4', borderRadius: 4 ,flex: 1,margin:2}}>
                        <Picker
...
<Picker.Item label="--Select--" value="0" key="0" />
        ...
        flexDirection: 'row',height:50,width:150,alignSelf:'center',justifyContent:'center',alignItems:'center'}} onPress={()=>setModalVisible(!modalVisible)}>
                    ...
                    <Text style={styles.titrePick}>Téléchargez les deux faces de votre carte d'identité nationale</Text>
                    <View style={styles.action}>
                        <TouchableOpacity style={[styles.continue,{flex: 1, margin:2}]} onPress={() => openImagePickerAsyncOne()}>
                            <Text style={styles.textSign} >Face 1</Text>
                            {pictureone != null ? 
                                <AntDesign style={{ marginLeft: 4}} name="checkcircle" size={20} color="green" /> :
                                null
                            }
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.continue,{flex: 1, margin:2}]} onPress={() => openImagePickerAsyncTwo()}>
                            <Text style={styles.textSign} >Face 2</Text>
                            {picturetwo != null ? 
                                <AntDesign style={{ marginLeft: 4}} name="checkcircle" size={20} color="green" /> :
                                null
                            }
